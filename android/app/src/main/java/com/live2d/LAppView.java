...
public class LAppView implements AutoCloseable {
    ...
    public LAppView(String instanceId) {
        Log.d(TAG, "LAppView constructor called with instanceId: " + instanceId);
    ...
    public void initialize() {
        Log.d(TAG, "initialize: Initializing LAppView for instance: " + instanceId);
        try {
            LAppDelegate appDelegate = LAppDelegate.getInstance(instanceId);
            if (appDelegate == null) {
                Log.e(TAG, "initialize: LAppDelegate is null for instance: " + instanceId);
                return;
            }
            ...
            // 防止除以零错误
            if (height <= 0 || width <= 0) {
                Log.e(TAG, "initialize: Invalid window size - width=" + width + ", height=" + height);
                return;
            }
            ...
            if (spriteShader == null) {
                ...
                Log.d(TAG, "initialize: Created new LAppSpriteShader");
            }
            
            Log.d(TAG, "initialize: LAppView initialization completed");
        } catch (Exception e) {
            Log.e(TAG, "initialize: Error during initialization", e);
        }
    }

    // 精灵初始化
    public void initializeSprite() {
        Log.d(TAG, "initializeSprite: Initializing sprites for instance: " + instanceId);
        try {
            LAppDelegate appDelegate = LAppDelegate.getInstance(instanceId);
            if (appDelegate == null) {
                Log.e(TAG, "initializeSprite: LAppDelegate is null for instance: " + instanceId);
                return;
            }
            
            int windowWidth = appDelegate.getWindowWidth();
            int windowHeight = appDelegate.getWindowHeight();
            Log.d(TAG, "initializeSprite: Window size - width=" + windowWidth + ", height=" + windowHeight);

            LAppTextureManager textureManager = appDelegate.getTextureManager();
            ...
            Log.e(TAG, "initializeSprite: Error during sprite initialization", e);
        }
    }
    ...
    public void render() {
        try {
            ...
            LAppDelegate appDelegate = LAppDelegate.getInstance(instanceId);
            if (appDelegate == null) {
                Log.e(TAG, "render: LAppDelegate is null for instance: " + instanceId);
                return;
            }
            
            int maxWidth = appDelegate.getWindowWidth();
            int maxHeight = appDelegate.getWindowHeight();
            
            // 防止无效尺寸
            if (maxWidth <= 0 || maxHeight <= 0) {
                Log.e(TAG, "render: Invalid window size - width=" + maxWidth + ", height=" + maxHeight);
                return;
            }
            ...
            if (isChangedModel) {
                ...
                if (manager != null) {
                    ...
                    Log.e(TAG, "render: LAppLive2DManager is null for instance: " + instanceId);
                }
            }
                ...
                if (renderingTarget == RenderingTarget.MODEL_FRAME_BUFFER && renderingSprite != null) {
                    ...
                    for (int i = 0; i < live2dManager.getModelNum(); i++) {
                        ...
                        if (model != null) {
                            ...
                            if (renderingSprite != null && model.getRenderingBuffer() != null) {
            ...
            Log.e(TAG, "render: Error during rendering", e);
        }
    }
        ...
        try {
            ...
            if (renderingTarget != RenderingTarget.NONE) {
                ...
                if (!useTarget.isValid()) {
                    LAppDelegate appDelegate = LAppDelegate.getInstance(instanceId);
                    if (appDelegate == null) {
                        Log.e(TAG, "preModelDraw: LAppDelegate is null for instance: " + instanceId);
                        return;
                    }
                    ...
                    if (width <= 0 || height <= 0) {
                        Log.e(TAG, "preModelDraw: Invalid window size - width=" + width + ", height=" + height);
                        return;
                    }
            ...
            Log.e(TAG, "preModelDraw: Error during pre-model draw", e);
        }
    }
        ...
        try {
            ...
            if (renderingTarget != RenderingTarget.NONE) {
                ...
                if (renderingTarget == RenderingTarget.VIEW_FRAME_BUFFER && renderingSprite != null) {
                    ...
                    LAppDelegate appDelegate = LAppDelegate.getInstance(instanceId);
                    if (appDelegate == null) {
                        Log.e(TAG, "postModelDraw: LAppDelegate is null for instance: " + instanceId);
                        return;
                    }
                    ...
                    if (maxWidth <= 0 || maxHeight <= 0) {
                        Log.e(TAG, "postModelDraw: Invalid window size - width=" + maxWidth + ", height=" + maxHeight);
                        return;
                    }
            ...
            Log.e(TAG, "postModelDraw: Error during post-model draw", e);
        }
    }
    ...
    public void onTouchesMoved(float pointX, float pointY) {
        ...
        if (manager != null) {
            ...
            Log.e(TAG, "onTouchesMoved: LAppLive2DManager is null for instance: " + instanceId);
        }
    }
        ...
        try {
            ...
            if (live2DManager != null) {
                ...
                if (isPowerVisible && powerSprite != null && powerSprite.isHit(pointX, pointY)) {
                    ...
                    LAppDelegate appDelegate = LAppDelegate.getInstance(instanceId);
                    if (appDelegate != null) {
                        appDelegate.deactivateApp();
                    } else {
                        Log.e(TAG, "onTouchesEnded: LAppDelegate is null for instance: " + instanceId);
                    }
                } else {
                ...
                Log.e(TAG, "onTouchesEnded: LAppLive2DManager is null for instance: " + instanceId);
            }
        } catch (Exception e) {
            Log.e(TAG, "onTouchesEnded: Error during touch end handling", e);
        }
    }
    ...
    private void requestRender() {
       ...
       if (appDelegate != null) {
           ...
           Log.d(TAG, "requestRender: Render requested for instance: " + instanceId);
           ...
           if (appDelegate.getActivity() != null) {
               ...
               appDelegate.getActivity().runOnUiThread(new Runnable() {
                   ...
                   public void run() {
                       // 再次请求渲染以确保UI更新
                       appDelegate.requestRender();
                       Log.d(TAG, "requestRender: Additional render request from UI thread for instance: " + instanceId);
                   }
               });
           }
       } else {
           Log.w(TAG, "requestRender: LAppDelegate is null for instance: " + instanceId);
       }
   }
